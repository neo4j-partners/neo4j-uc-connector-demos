...
`validate-federation` currently exercises from Databricks. It intentionally
...
loader uses the same CSV files and Cypher statements as
`sample-validation/scripts/run_00_load_graph.py`, but reads from
`getting-started/data/aircraft_digital_twin_data` and writes directly with the
Neo4j Python driver. It does not require Databricks, Spark, UC Volumes, or the
`sample-validation` upload/submit flow.

```bash
cd direct-jdbc-validation
cp .env.sample .env
...
`run_01_connect_test.py` is not required for these local direct-JDBC tests
because this project does not use Unity Catalog or Delta tables.
...
Run only the baseline validate-federation coverage:
...
`validate-federation/scripts/run_01_connection_validation.py` and
