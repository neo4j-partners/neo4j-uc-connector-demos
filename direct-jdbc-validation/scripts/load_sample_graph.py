...
This is the direct-JDBC validation equivalent of
sample-validation/scripts/run_00_load_graph.py. It uses the same CSV files and
Cypher load statements, but reads the local repository data files and connects
with the Neo4j Python driver instead of going through Databricks.
"""
...
DEFAULT_ENV_FILE = REPO_ROOT / "direct-jdbc-validation/.env"
...
def main() -> int:
    ...
    print("=" * 60)
    print("direct-jdbc-validation: Direct Python Sample Graph Load")
    print("=" * 60)
...
def build_config(args: argparse.Namespace, env: dict[str, str]) -> Config:
    ...
    if not password:
        raise SystemExit(
            "Missing NEO4J_PASSWORD. Set it in direct-jdbc-validation/.env "
            "or the environment."
        )
    if not uri:
        if not host:
            raise SystemExit(
                ...
                "direct-jdbc-validation/.env or the environment."
            )
