...
The `validation/` directory includes scripts that automate the prerequisite steps. Run these once before creating the connection.
...
|Workspace path where scripts are uploaded (e.g. `/Users/you@example.com/validation`)
...
|Name of the Databricks secret scope to create (e.g. `validation`)
...
ws = WorkspaceClient()
password = ws.dbutils.secrets.get(scope="validation", key="NEO4J_PASSWORD")
----

=== 3. Upload CSV Data to the UC Volume

`upload_data.sh` uploads the aircraft digital twin CSV files from `getting-started/data/aircraft_digital_twin_data/` to the UC Volume path defined by `UC_CATALOG`, `UC_SCHEMA`, and `UC_VOLUME` in `.env`:
...
* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/getting-started/01-simple-connect-test.ipynb[01-simple-connect-test.ipynb] -- runnable notebook that creates the UC JDBC connection and validates connectivity
