...
Fleet-wide overview combining Neo4j graph metrics with Delta sensor analytics in a single SQL statement using pure SQL federation via Neo4j Federated JDBC UC Connection.
...
* **Delta side** (1 subquery): Computes fleet-wide sensor averages across all 172K readings by sensor type
* **CROSS JOIN**: Combines the single-row results from both sides into one unified fleet summary

This shape follows the recommended federation pattern for the custom JDBC connection: push graph traversal and aggregation to Neo4j, aggregate lakehouse data in Spark, then combine the reduced results in a single Spark SQL statement.
...
+------------------------+---------------+-------------+--------------------------+---------------+-----------------+-----------------+----------------+--------------+
|300                     |0              |800          |800                       |671.6          |0.2659           |1.39             |4768.0          |172800       |
+------------------------+---------------+-------------+--------------------------+---------------+-----------------+-----------------+----------------+--------------+
....
