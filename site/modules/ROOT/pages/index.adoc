...
NOTE: The Neo4j JDBC driver requires additional memory for class loading in the Databricks SafeSpark sandbox. Without the correct Spark configuration (`spark.databricks.safespark.jdbcSandbox.jvm.maxMetaspace.mib`, etc.), the sandbox JVM will crash with "Connection was closed before the operation completed." See the full setup details in the https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/docs/neo4j_uc_jdbc_guide.md[Neo4j Federated JDBC UC Connection Setup Guide].

== The Demo: Aircraft Digital Twin

To demonstrate federated querying in practice, this guide uses a synthetic aerospace IoT dataset: an https://github.com/neo4j-field/databricks-neo4j-mcp-demo/tree/main/aircraft_digital_twin_data[aircraft digital twin] modeling a fleet of 20 aircraft over 45 days of operations.
...
|Sensor telemetry (172,800 readings), aircraft metadata, system and sensor catalogs
...
The https://github.com/neo4j-partners/neo4j-uc-connector-demos/tree/main/getting-started[getting-started notebooks] in the repo walk through the complete setup and query progression:

* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/getting-started/00-load-graph.ipynb[00-load-graph.ipynb] -- loads the aircraft digital twin dataset into Neo4j
* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/getting-started/01-neo4j-uc-connection-setup.ipynb[01-neo4j-uc-connection-setup.ipynb] -- creates the UC JDBC connection and validates connectivity
* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/getting-started/02-federated-queries.ipynb[02-federated-queries.ipynb] -- runs live federated queries combining Neo4j graph topology with Delta sensor data
* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/getting-started/03-materialized-tables.ipynb[03-materialized-tables.ipynb] -- materializes Neo4j node labels as Delta tables for Genie and SQL analytics
...
== xref:uc-integration-setup.adoc[Deep Dive: UC Integration Setup]
...
2. Neo4j Federated JDBC UC Connection configured (see https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/docs/neo4j_uc_jdbc_guide.md[setup guide])
3. Neo4j Spark Connector installed as cluster library (for queries 3-5)
4. Databricks secret scope configured via `cd validation && uv run python validate.py secrets` (default name: `neo4j-uc-demos`)

== References

* https://github.com/neo4j-partners/neo4j-uc-connector-demos/blob/main/docs/neo4j_uc_jdbc_guide.md[Neo4j Federated JDBC UC Connection Setup Guide]
