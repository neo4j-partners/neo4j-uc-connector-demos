...
The `sample-validation` directory includes scripts that automate the prerequisite steps. Run these once before creating the connection.
...
|Workspace path where scripts are uploaded (e.g. `/Users/you@example.com/sample_validation`)
...
|Name of the Databricks secret scope to create (e.g. `sample_validation`)
...
ws = WorkspaceClient()
password = ws.dbutils.secrets.get(scope="sample_validation", key="NEO4J_PASSWORD")
----
