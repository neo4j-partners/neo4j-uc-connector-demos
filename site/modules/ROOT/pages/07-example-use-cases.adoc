...
* **Cross-source aggregate joins**: Combine `remote_query()` graph traversal and aggregate results with Delta tables in one SQL statement. This keeps graph-side reduction in Neo4j and lakehouse aggregation in Databricks.
...
* Cross-source query combining `remote_query()` fraud indicators with Delta transaction aggregates for per-account risk scoring
...
* Cross-source query combining `remote_query()` aggregates for network-wide supplier concentration risk with lakehouse logistics metrics for per-component alternative sourcing analysis
...
* Cross-source query combining several `remote_query()` graph aggregates with lakehouse features to rapidly evaluate which Neo4j data is worth materializing long-term

== Learn More

* xref:index.adoc[Federated Query Patterns]: Live query examples combining Neo4j and Delta lakehouse data
* xref:uc-integration-setup.adoc[UC Integration Setup]: How to configure the Neo4j JDBC connection to Unity Catalog
